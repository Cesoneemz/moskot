...
class UserAccount(AbstractBaseUser, PermissionsMixin):
    ...
    education_institution = models.CharField(
        max_length=255, verbose_name="Место обучения"
    )

    GENDER_CHOICES = [("male", "Мужской"), ("female", "Женский")]

    gender = models.CharField(
        max_length=128, choices=GENDER_CHOICES, default="male", verbose_name="Пол"
    )

    CITIZENSHIP_CHOICES = [("rus", "Россия"), ("other", "Другое")]
    ...
    cv = models.FileField(
        upload_to=get_profile_picture_path, verbose_name="Резюме", blank=True, null=True
    )

    about_user = models.TextField(
        max_length=1024, verbose_name="О пользователе", blank=True, null=True
    )
...
class WorkExperience(models.Model):
    ...
    description = models.TextField(verbose_name="Описание опыт работы")
